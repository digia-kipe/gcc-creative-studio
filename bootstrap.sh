...
configure_environment() {
    ...
    if [ ! -d "$ENV_DIR" ]; then
        ...
        if [[ $REPLY =~ ^[Yy]$ ]]; then
            ...
            info "Creating GCS bucket '$BUCKET_NAME' for Terraform state..."; gcloud storage buckets create gs://${BUCKET_NAME} --project="$GCP_PROJECT_ID" --location="europe-west1" || warn "Bucket 'gs://${BUCKET_NAME}' may already exist. Continuing..."
        fi
